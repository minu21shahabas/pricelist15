...
def addpl(request):
    ...
    if request.method== 'POST':
        ...
        if radio=='rate':
            ...
            cust=request.POST['custom']
            name=request.POST['name']
            types=request.POST.get('type')
            desc=request.POST.get('desc')
            u=request.user.id
            user=User.objects.get(id=u)
            ad_item=Pricelist(name=name,types=types,currency=cur,description=desc,user=user,cust_rate=cust
                                )
                ...
                u=request.user.id
                user=User.objects.get(id=u)
                ad_item=Pricelist(name=name,types=types,currency='none',description=desc,user=user,mark=mark,percentage=perc,roundoff=rounds
                                )
